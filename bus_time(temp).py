...
def early():
    ...
    # print(bus23to65)
    # print(bus65to23)
    return (dt_now, year, month, day, week, bus23to65, bus65to23, info)

#GUI設定
class GUI():
    def __init__(self, info, next_time):
...
def next_bus_time(bus_Sche_time, hour, minute, second, info):
    ...
    bus_arr_time = ""
    if info == "":
        for i in range(time_Sche_num):
            #出発時刻
            #print(bus_Sche_time)
            Sche_depa_hour = bus_Sche_time[i][0]
            Sche_depa_min = bus_Sche_time[i][1]
            if time_Sche_num > i and (int(Sche_depa_hour) > hour or (int(Sche_depa_hour) == hour and int(Sche_depa_min) >= minute)):
                #到着予定時刻
...
def main():
    next_time = ""
    dt_now, year, month, day, week, bus23to65, bus65to23, info = early()
    gui = GUI(info, next_time)
    win = gui.window
    bus_Sche_time = ""
    while True:
